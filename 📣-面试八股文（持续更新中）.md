...
[闭包？什么是闭包？--JavaScript前端 - 掘金 (juejin.cn)](https://juejin.cn/post/7197189371370913850?searchId=202309061607067256875A42619188DD26)

[每日一题-什么是闭包？ - 掘金 (juejin.cn)](https://juejin.cn/post/6963860770643771428?from=search-suggest)
...
**1. 理解：**就是一块地盘，一个代码段所在的区域。它是静态的（相对于上下文对象），在编写代码时就确定了。直接写在script标签里的代码都是全局作用域。

**2. 分类：**全局作用域、局部作用域、块级作用域（ES6出的）

全局作用域：

- 直接编写在script标签中的JS代码都在全局作用域。 全局作用域在页面打开时创建，在页面关闭时销毁
...
**4. 有多少个作用域**：n+1原则，n表示定义了几个函数，1表示全局作用域
...
- 内部变量会覆盖外部变量
- 用来计数的循环变量泄漏为全局变量：`for` 循环中的用 `var` 定义的变量在外部作用域可以访问

`if(false)` 永远不会为真，所以这里面的语句一直不会执行。

**什么是块级作用域（ES6 提供 let & const 变量实现块级作用域）**

**使用：**

- ES6 允许块级作用域的**任意嵌套**。
- 内层作用域可以定义外层作用域的**同名变量**。
- `ES6` 引入了块级作用域，明确允许在块级作用域之中**声明函数**。`ES6` 规定，块级作用域之中，函数声明语句的行为类似于 `let`，在块级作用域之外不可引用。

**解决的问题：**

块级作用域的出现，实际上使得获得广泛应用的匿名立即执行函数表达式（匿名 `IIFE`）不再必要了。

[ES6 块级作用域详解_es6块级作用域_wflynn的博客-CSDN博客](https://blog.csdn.net/weixin_40013817/article/details/101198512)
...
- 类：让JavaScript的面向对象编程变得更加简单和易于理解
...
[ES6、ES7、ES8、ES9、ES10新特性一览 - 掘金 (juejin.cn)](https://juejin.cn/post/6844903811622912014#heading-22)
...
[彻底搞懂var、let、const的区别 - 掘金 (juejin.cn)](https://juejin.cn/post/7202996449775222844)
